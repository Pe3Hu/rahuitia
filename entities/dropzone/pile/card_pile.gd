...
@export var play_pile_position = Vector2(1250, 460)
@export var discard_pile_position = Vector2(1550, 460)
...
@export var play_stack_display_gap := 48
@export var play_face_up := true
@export var play_pile_layout := PilesCardLayouts.down


var card_database := [] # an array of JSON `Card` data
var card_collection := [] # an array of JSON `Card` data

var _draw_pile := [] # an array of `Card`s
var _hand_pile := [] # an array of `Card`s
var _play_pile := [] # an array of `Card`s
var _discard_pile := [] # an array of `Card`s
...
func set_card_pile(card : Card, pile : Piles):
	...
	if pile == Piles.play_pile:
		_play_pile.push_back(card)
		emit_signal("play_pile_updated")
...
func get_card_in_pile_at(pile : Piles, index : int):
	if pile == Piles.play_pile and _play_pile.size() > index:
		return _play_pile[index]
...
func get_card_pile_size(pile : Piles):
	if pile == Piles.play_pile:
		return _play_pile.size()
	elif pile == Piles.discard_pile:
...
func _maybe_remove_card_from_any_piles(card : Card):
	if _hand_pile.find(card) != -1:
		...
		emit_signal("draw_pile_updated")
	elif _play_pile.find(card) != -1:
		_play_pile.erase(card)
		emit_signal("play_pile_updated")
	elif _discard_pile.find(card) != -1:
		_discard_pile.erase(card)
		emit_signal("discard_pile_updated")
	
func create_card_in_dropzone(card_resource_: CardResource, dropzone : CardDropzone):
	var card = _create_card(card_resource_)
	card.position = dropzone.position
	set_card_dropzone(card, dropzone)
	
func create_card_in_pile(card_resource_: CardResource, pile_to_add_to : Piles):
	var card = _create_card(card_resource_)
	if pile_to_add_to == Piles.hand_pile:
		card.position = hand_pile_position
	if pile_to_add_to == Piles.play_pile:
		card.position = play_pile_position
	...
	set_card_pile(card, pile_to_add_to)
...
func init_core_cards():
	for card_core_resourec in default_core_deck.core_cards:
		var card = _create_card(card_core_resourec)
		_draw_pile.push_back(card)
		_draw_pile.shuffle()
...
func _set_discard_pile_target_positions():
	for i in _discard_pile.size():
		var card = _discard_pile[i]
		var target_pos = discard_pile_position
		if discard_pile_layout == CardPile.PilesCardLayouts.up:
			if i <= max_stack_display:
				target_pos.y -= i * stack_display_gap
			else:
				target_pos.y -= stack_display_gap * max_stack_display
			...
			if i <= max_stack_display:
				...
				target_pos.y += stack_display_gap * max_stack_display
		elif discard_pile_layout == CardPile.PilesCardLayouts.right:
			if i <= max_stack_display:
				target_pos.x += i * stack_display_gap
			else:
				target_pos.x += stack_display_gap * max_stack_display
		elif discard_pile_layout == CardPile.PilesCardLayouts.left:
			if i <= max_stack_display:
				target_pos.x -= i * stack_display_gap
			else:
				target_pos.x -= stack_display_gap * max_stack_display
...
func _set_play_pile_target_positions():
	for i in _play_pile.size():
		var card = _play_pile[i]
		var target_pos = play_pile_position
		
		match play_pile_layout:
			CardPile.PilesCardLayouts.up:
				if i <= max_stack_display:
					target_pos.y -= i * play_stack_display_gap
				else:
					target_pos.y -= play_stack_display_gap * max_stack_display
			CardPile.PilesCardLayouts.down:
				if i <= max_stack_display:
					target_pos.y += i * play_stack_display_gap
				else:
					target_pos.y += play_stack_display_gap * max_stack_display
			CardPile.PilesCardLayouts.right:
				if i <= max_stack_display:
					target_pos.x += i * play_stack_display_gap
				else:
					target_pos.x += play_stack_display_gap * max_stack_display
			CardPile.PilesCardLayouts.left:
				if i <= max_stack_display:
					target_pos.x -= i * play_stack_display_gap
				else:
					target_pos.x -= play_stack_display_gap * max_stack_display
...
func is_card_in_hand(card):
	return _hand_pile.filter(func(c): return c == card).size()
	
func is_any_card_clicked():
	for card in _hand_pile:
		if card.is_clicked:
			return true
	var all_dropzones := []
	_get_dropzones(get_tree().get_root(), "CardDropzone", all_dropzones)
...
func _create_card(card_resource_: CardResource):
	...
	card.connect("card_hovered", func(c): emit_signal("card_hovered", c))
	card.connect("card_unhovered", func(c): emit_signal("card_unhovered", c))
	card.connect("card_clicked", func(c): emit_signal("card_clicked", c))
	card.connect("card_dropped", func(c): emit_signal("card_dropped", c))
	add_child(card)
	return card
	
#func _get_card_data_by_name(name_ : String):
