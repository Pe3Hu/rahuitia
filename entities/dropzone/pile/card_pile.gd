...
func create_card_in_pile(card_resource_: CardResource, pile_type_: FrameworkSettings.PileType) -> void:
	var card = create_card(card_resource_)
...
func init_core_cards() -> void:
	for card_core_resourec in default_core_deck.core_cards:
		var card = create_card(card_core_resourec)
...
func draw(num_cards: int = 1) -> void:
	for _i in num_cards:
		...
		if draw_cards.size():
			...
			var dUPe_discard = discard_cards.duplicate()
			for c in dUPe_discard: # you can't remove things from the thing you loop!!
				set_card_pile(c, FrameworkSettings.PileType.DRAW)
			draw_cards.shuffle()
			set_card_pile(draw_cards[draw_cards.size() - 1], FrameworkSettings.PileType.HAND)
...
func create_card(card_resource_: CardResource) -> Card:
