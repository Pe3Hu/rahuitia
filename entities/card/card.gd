...
func _card_can_be_interacted_with():
	...
	if parent is CardPile:
		...
		if parent.is_card_in_hand(self):
			valid = parent.is_hand_enabled() and not parent.is_any_card_clicked()
		# check for cards in dropzone
		var dropzone = parent.get_card_dropzone(self)
		if dropzone:
			valid = dropzone.get_top_card() == self and not parent.is_any_card_clicked()
	return valid
...
func _on_gui_input(event):
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			...
			if parent is CardPile and parent.get_card_pile_size(CardPile.Piles.draw_pile) > 0 and parent.is_hand_enabled() and parent.get_cards_in_pile(CardPile.Piles.draw_pile).find(self) != -1 and not parent.is_any_card_clicked() and parent.click_draw_pile_to_draw:
				parent.draw(1)
		else:
			#event released
			if is_clicked:
